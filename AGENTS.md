...
`apps/site/platform.json` is the complete list of what lvwwd.org needs in production. When a change
makes the Worker read a new binding, secret, or var, add it to that file in the same change, with
its purpose and the steps to find its value. Add any value that only the workers.dev previews may
carry to its `forbidden` list. Never set or change a production secret from an agent session; a
maintainer runs `pnpm bootstrap --production`.
