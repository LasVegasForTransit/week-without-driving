...
- `apps/site/wrangler.jsonc` names the Worker, its two custom domains, and its daily cleanup, which
  deletes all participant data from November 30, 2026
- `apps/site/wrangler.api-preview.jsonc` is the API preview on workers.dev. It has its own database,
  shows each "Open my week" link on the page, and treats every day as Day 3 of the week. Deploy it
